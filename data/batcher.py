...
def convert_batch_extract_ptr(unk, word2id, batch):
    def convert_one(sample):
        source_sents, extracts = sample
        # source_sents = [sent for sent in source_sents if sent[0] != '###' and not is_quote(sent)]
        source_sents = [sent for sent in source_sents if not is_quote(sent, keep_hash=True)]
        # print('len(source_sents) extracts', len(source_sents), extracts)
