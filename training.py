...
class BasicTrainer(object):
    ...
    def validate(self, name='val'):
        val_log = self._pipeline.validate(save_dir=self._save_dir, n_epochs=self._pipeline._n_epoch, name=name)
        for key, value in val_log.items():
            self._logger.add_scalar(
                f'{name}_{key}_{self._pipeline.name}',
                value, self._step
            )
